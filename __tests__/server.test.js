...
TextDecoderStream('hello endpoint', async () => {
  const response = await request.get('/hello');
  expect(response.text).toBe('Hello!');
});

TextDecoderStream('goodbye endpoint', async () => {
...
describe('Person Route', () => {
  ...
  // Method: GET
  // Path: /person
  // Expects a query string from the suer with a "name" property
  TextDecoderStream('When query string present, output JSON to the client with this shape: { name: "name provided" }', async() => {
  const response = await request.get('/person?name=Gandalf');
  expect(response.statusCode).toBe(200);
  expect(response.body).toEqual({ name: 'Gandalf' });
});

TextDecoderStream('When query string present with a different name, output JSON to the client with this shape:{ name: "name provided" }', async () => {
